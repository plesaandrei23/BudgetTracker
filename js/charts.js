...
export function renderAnalytics() {
    ...
    if (insightsDiv) {
        insightsDiv.innerHTML = `
            <!-- NET WORTH CARD -->
            <div class="bg-amber-500 rounded-3xl p-6 shadow-lg shadow-amber-500/20 text-center relative overflow-hidden">
                <div class="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -mr-16 -mt-16 blur-2xl"></div>
                <div class="text-amber-900/80 text-xs font-bold uppercase tracking-widest mb-1">Rezultat Net (${formatDateRange(refDate, mode)})</div>
                <div class="text-3xl font-extrabold text-black mb-4 relative z-10">${net.toFixed(2)} <span class="text-sm font-normal opacity-70">RON</span></div>
                
                <div class="flex justify-center gap-2 relative z-10">
                    <div class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2 border border-black/5">
                        <div class="w-2 h-2 rounded-full bg-emerald-600"></div>
                        <div class="text-left">
                            <div class="text-[10px] text-amber-900/70 font-bold">VENITURI</div>
                            <div class="text-sm font-bold text-black">${income.toFixed(0)}</div>
                        </div>
                    </div>
                    <div class="bg-white/20 backdrop-blur-sm px-4 py-2 rounded-xl flex items-center gap-2 border border-black/5">
                        <div class="w-2 h-2 rounded-full bg-red-600"></div>
                        <div class="text-left">
                            <div class="text-[10px] text-amber-900/70 font-bold">CHELTUIELI</div>
                            <div class="text-sm font-bold text-black">${expense.toFixed(0)}</div>
                        </div>
                    </div>
                </div>
            </div>
