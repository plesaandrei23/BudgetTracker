...
export function renderAccounts() {
    ...
    let html = `
        <div onclick="selectAccount('all')" class="flex-shrink-0 w-36 h-24 bg-slate-800 rounded-2xl p-4 border ${state.selectedAccount === 'all' ? 'border-blue-500' : 'border-slate-700'} flex flex-col justify-between cursor-pointer snap-start">
            <div class="text-xs text-slate-400 uppercase font-bold">Total Net Worth</div>
            <div class="text-lg font-bold text-white">${total.toFixed(2)}</div>
        </div>
    `;
    ...
    state.userConfig.accounts.forEach(acc => {
        ...
        html += `
            <div onclick="selectAccount('${acc.id}')" class="flex-shrink-0 w-36 h-24 bg-slate-800 rounded-2xl p-4 border ${isSel ? 'border-blue-500' : 'border-slate-700'} flex flex-col justify-between cursor-pointer snap-start">
                <div class="flex justify-between items-start">
                    <span class="text-xs text-slate-400 uppercase font-bold truncate pr-2">${acc.name}</span>
                    <i class="fa-solid ${acc.icon} ${acc.color}"></i>
                </div>
                <div class="text-lg font-bold text-white">${bal.toFixed(2)}</div>
            </div>
        `;
    });
...
export function renderTransactions() {
    ...
    Object.values(groups).forEach(group => {
        ...
        header.className = "sticky top-0 bg-rev-bg/95 backdrop-blur z-10 py-2 px-1 text-xs font-bold text-slate-500 uppercase";
        header.innerText = group.label;
        list.appendChild(header);

        const groupDiv = document.createElement('div');
        groupDiv.className = "bg-slate-800/50 rounded-2xl overflow-hidden mb-4 border border-white/5";
        ...
        group.items.forEach(t => {
            ...
            if (isExp) {
                ...
                color = 'text-blue-500'; // Transfer
                icon = 'fa-arrow-right-arrow-left';
            }

            const div = document.createElement('div');
            div.className = "p-4 flex items-center gap-4 hover:bg-white/5 transition border-b border-white/5 last:border-0 cursor-pointer";
            ...
            div.innerHTML = `
                ...
                <div class="flex-1">
                    <div class="flex justify-between">
                        <span class="font-bold text-sm text-slate-200">${t.description || t.category}</span>
                        <span class="font-bold text-sm ${color}">${sign}${t.amount.toFixed(2)}</span>
                    </div>
                     <div class="flex justify-between mt-0.5">
                        <span class="text-xs text-slate-500">${t.category}</span>
                        ${t.tipAmount ? `<span class="text-[10px] bg-purple-500/20 text-purple-300 px-1 rounded">Tip: ${t.tipAmount}</span>` : ''}
                    </div>
                </div>
            `;
...
export function setType(type) {
    ...
    Object.keys(map).forEach(k => {
        ...
        if (k === type) el.className = "flex-1 py-2 rounded-lg text-sm font-bold bg-rev-card shadow text-white transition-all";
        else el.className = "flex-1 py-2 rounded-lg text-sm font-bold text-slate-500 transition-all";
    });
...
function renderCategoryList(type) {
    ...
    cats.forEach(c => {
        ...
        btn.className = `flex-shrink-0 px-4 py-1.5 rounded-full text-xs font-bold border transition ${isActive ? 'bg-blue-600 text-white border-transparent' : 'bg-slate-700 text-slate-300 border-transparent'}`;
        btn.innerText = c;
        btn.onclick = (e) => {
...
export function renderSettingsUI() {
    ...
    div.className = "bg-rev-card rounded-2xl overflow-hidden border border-white/5 mt-4";
    div.innerHTML = `
        <div class="p-4 border-b border-white/5 font-bold text-sm text-slate-400 uppercase">Configuration</div>
        <button onclick="addConfigItem('expenseCats')" class="w-full flex justify-between p-4 hover:bg-white/5 transition border-b border-white/5">
            <span>Add Expense Category</span> <i class="fa-solid fa-plus text-blue-500"></i>
        </button>
        <button onclick="addConfigItem('incomeCats')" class="w-full flex justify-between p-4 hover:bg-white/5 transition border-b border-white/5">
            <span>Add Income Category</span> <i class="fa-solid fa-plus text-blue-500"></i>
        </button>
        <button onclick="addConfigItem('accounts')" class="w-full flex justify-between p-4 hover:bg-white/5 transition">
            <span>Add New Account</span> <i class="fa-solid fa-plus text-blue-500"></i>
        </button>
    `;
...
window.addConfigItem = async (key) => {
    ...
    if (key === 'accounts') {
        ...
        newConfig.accounts.push({ id, name, type: 'bank', icon: 'fa-piggy-bank', color: 'text-white' });
    } else {
